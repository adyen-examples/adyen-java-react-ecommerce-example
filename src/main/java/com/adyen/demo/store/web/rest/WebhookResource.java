        ...
        for (NotificationRequestItem item : notificationRequestItems) {
            ...
            switch (item.getEventCode()) {
                ...
                case NotificationRequestItem.EVENT_CODE_AUTHORISATION:
                    {
                        ShoppingCart cart = shoppingCartService
                            .findOneByPaymentReference(item.getPspReference())
                ...
                case NotificationRequestItem.EVENT_CODE_CANCEL_OR_REFUND:
                    ...
                    ShoppingCart cart = shoppingCartService
                        .findOneByPaymentModificationReference(item.getPspReference())
                        .orElseThrow(() -> new EntityNotFoundException("Shopping cart"));
                    if (item.isSuccess()) {
                        // update DB with additionalData.modification.action
                        if (
                            item.getAdditionalData().containsKey("modification.action") &&
                            "refund".equals(item.getAdditionalData().get("modification.action"))
                        ) {
